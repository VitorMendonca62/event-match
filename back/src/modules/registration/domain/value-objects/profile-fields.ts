export class DisplayName {
  ...
  static create(value: string): DisplayName {
    ...
    if (normalized.length < 1 || normalized.length > 60) throw new Error('Invalid display name.');
...
export class Region {
  ...
  static create(value: string): Region {
    const normalized = value.trim();
    if (normalized.length < 2 || normalized.length > 80) throw new Error('Invalid region.');
...
export class UsageIntent {
  ...
  static create(value: string): UsageIntent {
    if (!(USAGE_INTENTS as readonly string[]).includes(value)) throw new Error('Invalid usage intent.');
